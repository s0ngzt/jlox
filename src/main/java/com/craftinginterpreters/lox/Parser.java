...
class Parser {
  ...
  private Stmt.Function function(String kind) {
    Token name = consume(IDENTIFIER, "Expect " + kind + " name."); // function name
    consume(LEFT_PAREN, "Expect '(' after " + kind + " name."); // (
    List<Token> parameters = new ArrayList<>(); // parameters
    if (!check(RIGHT_PAREN)) {
      do {
        ...
        parameters.add(consume(IDENTIFIER, "Expect parameter name.")); // parameter name
      } while (match(COMMA)); // ,
    }
    consume(RIGHT_PAREN, "Expect ')' after parameters."); // )

    consume(LEFT_BRACE, "Expect '{' before " + kind + " body."); // {
    List<Stmt> body = block(); // function body
